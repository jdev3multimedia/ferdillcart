   ...
   $keys = ['landing_page_text'  ,'landing_page_links','speciality','join_as_images','download_app_section','counter_section',
    ...
    switch ($tab) {
        ...
        case 'payment':
            $digital_payment_methods_count = Setting::whereIn('settings_type', ['payment_config'])
                ->whereIn('key_name', ['ssl_commerz', 'paypal', 'stripe', 'razor_pay', 'senang_pay', 'paytabs', 'paystack', 'paymob_accept', 'paytm', 'flutterwave', 'liqpay', 'bkash', 'mercadopago','wipay']) //#update2026
                ->where('is_active', 1)
                ->count();
    ...
    public function payment_index(Request $request)
    {
        ...
        $data_values = Setting::whereIn('settings_type', ['payment_config'])
            ->whereIn('key_name', ['ssl_commerz', 'paypal', 'stripe', 'razor_pay', 'senang_pay', 'paytabs', 'paystack', 'paymob_accept', 'paytm', 'flutterwave', 'liqpay', 'bkash', 'mercadopago','wipay'])
    ...
    public function payment_update(Request $request, $name)
    {
        ...
        } elseif ($name == 'wipay') { //#wipay_update2026
            Helpers::businessUpdateOrInsert(['key' => 'wipay'], [
                'value' => json_encode([
                    'status' => $request['status'],
                    'mode' => $request['mode'],
    ...
    public function payment_config_update(Request $request)
    {
        ...
        // #wipay_update2026
        $validation = [
            'gateway' => 'required|in:ssl_commerz,paypal,stripe,razor_pay,senang_pay,paytabs,paystack,paymob_accept,paytm,flutterwave,liqpay,bkash,mercadopago,wipay',
        ...
        if ($request['gateway'] == 'ssl_commerz') {
         ...
         // #wipay_update2026
        elseif ($request['gateway'] == 'wipay') {
            $additional_data = [
                'status' => 'required|in:1,0',
                'account_number' => 'required_if:status,1',
                'country_code' => 'required_if:status,1',
                'currency' => 'required_if:status,1',
            ];
        }
